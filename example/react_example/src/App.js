    ...
    <div className="App">
      <header className="App-header">
        ...
        <div className="buttons">
          <button className="connectButton" disabled={!connected} onClick={getAccounts}>Get All Accounts</button>
          <button className="connectButton" onClick={connectWallet}>Connect Wallet</button>
          <button className="connectButton" disabled={!connected} onClick={getCurrentAccount}>Get Current Account</button>
        </div>

        <div className="console-output">
            {logs.map((log, index) => {
                return <p key={index} className={log.type}>{log.message}</p>
            })}
        </div>
      </header>
    </div>
