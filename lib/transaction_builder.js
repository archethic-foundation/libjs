...
module.exports = class TransactionBuilder {
  ...
  toJSON() {
    return JSON.stringify({
      ...
      data: {
        ...
        keys: this.data.keys.map(({secret, authorizedKeys }) => {
          return {
  ...
  previousSignaturePayload() {
    ...
    const keysBuffer = this.data.keys.map(({ secret, authorizedKeys }) => {
    ...
    return concatUint8Arrays([
      ...
      bufContentSize,
      this.data.content,
      Uint8Array.from([this.data.keys.length]),
      concatUint8Arrays(keysBuffer),
