...
module.exports = class TransactionBuilder {
    ...
    authorizedKeys = authorizedKeys.map(({ publicKey, encryptedSecretKey }) => {
      ...
      return { publicKey: publicKey, encryptedSecretKey: encryptedSecretKey }
		...
		this.data.keys.authorizedKeys.push(authorizedKeys)
		...
		return JSON.stringify({
			...
			data: {
				...
				keys: {
					...
					authorizedKeys: this.data.keys.authorizedKeys.map(keysBySecret => {

            return keysBySecret.map(({ publicKey: publicKey, encryptedSecretKey: encryptedSecretKey }) => {
              return { 
	...
	previousSignaturePayload() {
		...
		const authorizedKeysBuffer = this.data.keys.authorizedKeys.map(authorizedKeysBySecret => {
			const nbAuthorizedKeys = authorizedKeysBySecret.length
			let buf = [Uint8Array.from([nbAuthorizedKeys])]

      authorizedKeysBySecret.forEach(({ publicKey, encryptedSecretKey }) => {     
				buf.push(publicKey)
