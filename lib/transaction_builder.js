...
const { isHex, hexToUint8Array, uint8ArrayToHex, concatUint8Arrays, encodeInt32, encodeInt64, toBigInt, toByteArray } = require("./utils")
...
module.exports = class TransactionBuilder {
   ...
   * @param {String} txType Transaction type ("keychain_access", "keychain", "transfer", "hosting", "code_proposal", "code_approval", "token")
   */
  constructor(txType) {
   ...
   * @param {String | Uint8Array} to Address of the recipient (hexadecimal or binary buffer)
   * @param {Float} amount Amount of UCO to transfer
   * @param {String | Uint8Array} token_address Address of token to spend (hexadecimal or binary buffer)
   * @param {Integer} token_id ID of the token to use (default to 0)
   */
  addTokenTransfer(to, amount, token_address, token_id = 0) {
    if (typeof (to) !== "string" && !(to instanceof Uint8Array)) {
      throw "'to' must be a string or Uint8Array"
    }

    if (typeof (token_address) !== "string" && !(token_address instanceof Uint8Array)) {
      throw "'token_address' must be a string or Uint8Array"
    }
    ...
    if (typeof (token_address) == "string") {
      if (!isHex(token_address)) {
        throw "'token_address' must be in hexadecimal form if it's string"
      }
      token_address = hexToUint8Array(token_address)
    }

    if (isNaN(token_id) && token_id < 0) {
      throw "'token_id' must be a valid integer >= 0"
    }
  ...
  previousSignaturePayload() {
    ...
    const tokenTransfersBuffers = this.data.ledger.token.transfers.map(function (transfer) {
      return concatUint8Arrays([
        ...
        transfer.to,
        encodeInt64(transfer.amount),
        Uint8Array.from([transfer.token_id])
      ])
