    ...
    * @param {String | ArrayBuffer} secret Secret encrypted (hexadecimal or binary buffer)
    ...
    addNFTTransfer(to, amount, nft_address) {
        ...
        if (typeof(nft_address) == "string") {
            ...
            nft_address = Buffer.from(nft_address, "hex")
        }
    ...
    previousSignaturePayload() {
        ...
        const ucoTransfersBuffers = this.data.ledger.uco.transfers.map(function (transfer) {
            ...
            amount_buf.writeDoubleBE(transfer.amount, 0)
        ...
        return Buffer.concat([
            ...
            Buffer.concat(ucoTransfersBuffers),
            Buffer.alloc(1, this.data.ledger.nft.transfers.length),
            Buffer.concat(nftTransfersBuffers),
...
function hexAuthorizedKeys(authorizedKeys) {
    ...
    for (const publicKey in authorizedKeys) {
        authorizedKeysHex[publicKey.toString('hex')] = authorizedKeys[publicKey].toString('hex')
    }
