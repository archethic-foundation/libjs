...
module.exports = class TransactionSender {
  ...
  async send(tx, endpoint, confirmationThreshold = 100, timeout = 60) {
    if (typeof (tx) !== "object" || tx.constructor.name !== 'TransactionBuilder') {
    ...
    if (typeof (timeout) !== 'number') {
      throw "'timeout' must be an integer"
    }
    ...
    // Create function to handle confirmation
    const handleConfirmation = function (nbConfirmations, maxConfirmations) {
    ...
    const handleError = function (context, reason) {
      clearTimeout(this.timeout)
    ...
    try {
      this.confirmationNotifier = await waitConfirmations(txAddress, this.absintheSocket, handleConfirmation)
      this.errorNotifier = await waitError(txAddress, this.absintheSocket, handleError)
    } catch (err) {
      this.onError.forEach(func => func(senderContext, err))
      return this
    }

    const handleSend = function (response) {
      if (response.status >= 200 && response.status <= 299) {
        ...
        response.json().then(err => this.onError.forEach(func => func(senderContext, err)))
      }
    }.bind(this)
