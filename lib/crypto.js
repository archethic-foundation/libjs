...
function generateDeterministicKeyPair(pvKey, curve, originId) {
  ...
  const originIdBuf = Uint8Array.from([originId])
  
  switch (curve) {
    case "ed25519": 
      var curveIdBuf = Uint8Array.from([0])
      var key = ec_eddsa.keyFromSecret(pvKey)
      var pubBuf = new Uint8Array(key.pubBytes())

      return {
        privateKey: concatUint8Arrays([curveIdBuf, originIdBuf, pvKey]),
        publicKey: concatUint8Arrays([curveIdBuf, originIdBuf, pubBuf])
      }
    case "P256":
      var curveIdBuf = Uint8Array.from([1])
    ...
    case "secp256k1":
      var curveIdBuf = Uint8Array.from([2])
