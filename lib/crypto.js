...
function generateDeterministicKeyPair(pvKey, curve, originId) {
  ...
  switch (curve) {
    case "ed25519": 
      curveIdBuf = Uint8Array.from([0])
      key = ec_eddsa.keyFromSecret(pvKey)
    ...
    case "P256":
      curveIdBuf = Uint8Array.from([1])
      key = ec_P256.keyFromPrivate(pvKey)
    ...
    case "secp256k1":
      curveIdBuf = Uint8Array.from([2])
      key = ec_secp256k1.keyFromPrivate(pvKey)
      pubBuf = hexToUint8Array(key.getPublic().encode("hex"))
      return {
        privateKey: concatUint8Arrays([curveIdBuf, originIdBuf, pvKey]),
        publicKey: concatUint8Arrays([curveIdBuf, originIdBuf, pubBuf])
