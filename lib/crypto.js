...
module.exports.generateDeterministicKeyPair = generateDeterministicKeyPair

module.exports.hashAlgoToID = hashAlgoToID
module.exports.IDToHashAlgo = IDToHashAlgo
module.exports.curveToID = curveToID
module.exports.IDToCurve = IDToCurve

/**
 * Get the ID of a given hash algorithm
 * @params {String} hashAlgo Hash algorithm
 */
function hashAlgoToID(hashAlgo) {
  switch(hashAlgo) {
    case "sha256":
      return 0
    case "sha512":
...
function getHashDigest(content, algo) {
  switch(algo) {
    case "sha256":
...
*/
function generateDeterministicKeyPair(pvKey, curve, originID) {
...
function getKeypair(pvKey, curve) {
  switch (curve) {
    case "ed25519":
      var key = ec_eddsa.keyFromSecret(pvKey)
      var pubBuf = new Uint8Array(key.pubBytes())
    ...
    case "P256":
      var key = ec_P256.keyFromPrivate(pvKey)
      var pubBuf = hexToUint8Array(key.getPublic().encode("hex"))
      return {
        privateKey: pvKey,
        publicKey: pubBuf
      }
    case "secp256k1":
      var key = ec_secp256k1.keyFromPrivate(pvKey)
      var pubBuf = hexToUint8Array(key.getPublic().encode("hex"))
      return {
        privateKey: pvKey,
        publicKey: pubBuf
...
function sign(data, privateKey) {
  ...
  switch (curveBuf[0]) {
    case 0:
      var key = ec_eddsa.keyFromSecret(pvBuf)
      return Uint8Array.from(key.sign(data).toBytes())
    case 1:
      var msgHash = createHash("sha256")
        .update(data)
        .digest()
...
function verify(sig, data, publicKey) {
  ...
  switch (curveBuf[0]) {
    case 0:
      var key = ec_eddsa.keyFromPublic(Array.from(pubBuf))
    ...
    case 1:
      ...
      var key = ec_P256.keyFromPublic(pubBuf)
    ...
    case 2: 
      ...
      var key = ec_secp256k1.keyFromPublic(pubBuf)
      return key.verify(msgHash, sig)
    default:
      throw "Curve not supported"
  }  
...
function ecEncrypt(data, publicKey) {
  ...
  switch (curve_buf[0]) {
    ...
    case 1: 
      var ecdh = createECDH("prime256v1")
      ecdh.generateKeys(); 
      var sharedKey = ecdh.computeSecret(pubBuf)
...
function ecDecrypt(ciphertext, privateKey) {
  ...
  switch (curve_buf[0]) {
    case 0:
      var ephemeralPubKey = ciphertext.slice(0, 32)
      var tag = ciphertext.slice(32, 32+16)
      var encrypted = ciphertext.slice(32+16, ciphertext.length)
    ...
    case 1:
      var ephemeralPubKey = ciphertext.slice(0, 65)
      var tag = ciphertext.slice(65, 65+16)
      var encrypted = ciphertext.slice(65+16, ciphertext.length)

      var ecdh = createECDH("prime256v1")
      ecdh.setPrivateKey(pvBuf)
      var sharedKey = ecdh.computeSecret(ephemeralPubKey)
      var { aesKey, iv } = deriveSecret(sharedKey)
...
function derivePrivateKey(seed, index) {
  ...
  const masterEntropy = hash.slice(32, 64)
