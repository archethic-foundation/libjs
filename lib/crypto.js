...
import { isHex, hexToUint8Array, concatUint8Arrays, encodeInt32, maybeHexToUint8Array, maybeStringToUint8Array, uint8ArrayToHex, wordArrayToUint8Array } from './utils.js';
...
function deriveSecret(sharedKey) {
  ...
  const pseudoRandomKey = wordArrayToUint8Array(CryptoJS.SHA256(sharedKey))

  const iv = wordArrayToUint8Array(
    CryptoJS.HmacSHA256("0", CryptoJS.lib.WordArray.create(pseudoRandomKey))
  ).subarray(0, 32)

  const aesKey = wordArrayToUint8Array(
    CryptoJS.HmacSHA256("1", CryptoJS.lib.WordArray.create(pseudoRandomKey))
  ).subarray(0, 32)
...
function aesAuthEncrypt(data, aesKey, iv) {
  ...
  const encrypted = sjcl.mode.gcm.encrypt(new sjcl.cipher.aes(keyBits), dataBits, ivBits)

  let cipherData = encrypted.slice(0)
  const w = sjcl.bitArray, cipherLength = w.bitLength(cipherData), tagLength = 128
  let tag;

  // Slice tag out of data
  if (tagLength <= cipherLength) {
    tag = w.bitSlice(cipherData, cipherLength - tagLength);
    cipherData = w.bitSlice(cipherData, 0, cipherLength - tagLength);
  } else {
    tag = cipherData;
    cipherData = [];
  }
  ...
  return {
    encrypted: hexToUint8Array(sjcl.codec.hex.fromBits(cipherData)),
...
function aesAuthDecrypt(encrypted, aesKey, iv, tag) {
  ...
  const ciphertextBits = sjcl.bitArray.concat(encryptedBits, tagBits)
  const decrypted = sjcl.mode.gcm.decrypt(new sjcl.cipher.aes(aesKeyBits), ciphertextBits, ivBits)
