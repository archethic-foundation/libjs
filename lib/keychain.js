...
export default class Keychain {
  constructor(core, seed, version = 1) {
  ...
  removeAuthorizedPublicKey(key) {
    this.authorizedPublicKeys = this.authorizedPublicKeys.filter(k => {
      // javascript can't compare object so we compare strings
  ...
  async update(timeoutSeconds = 5) {
    const keychainGenesisAddress = deriveAddress(this.seed, 0);
    const transactionChainIndex = await this.core.transaction.getTransactionIndex(keychainGenesisAddress);

    const aesKey = randomSecretKey();

    return new Promise((resolve, reject) => {
      new this.core.transaction.builder(this.core)
        .setType("keychain")
        .setContent(JSON.stringify(this.toDID()))
        .addOwnership(
          aesEncrypt(this.encode(), aesKey),
          this.authorizedPublicKeys.map(publicKey => {
            return { publicKey: publicKey, encryptedSecretKey: ecEncrypt(aesKey, publicKey) }
          })
        )
        .build(this.seed, transactionChainIndex)
  ...
  static decode(core, binary) {
    ...
    let keychain = new Keychain(core, seed, decodeInt32(version));
