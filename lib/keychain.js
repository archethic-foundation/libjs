...
class Keychain {
  ...
  buildTransaction(transaction, service, index) {
    ...
    const payloadForPreviousSignature = transaction.previousSignaturePayload()
    const previousSignature = sign(payloadForPreviousSignature, keypair.privateKey)
    
    transaction.setPreviousSignatureAndPreviousPublicKey(previousSignature, keypair.publicKey)
  }
